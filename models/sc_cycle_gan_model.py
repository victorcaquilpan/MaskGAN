...
class ScCycleGanModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'D_B', 'G_B', 'cycle_B', 'structure']
        ...
        if self.isTrain:  # define discriminators
            ...
            self.criterion_consistency = torch.nn.L1Loss()
            # Define MIND feature extractor
            self.feature_extractor = networks.MIND().to(self.device)
    ...
    def backward_G(self):
        ...
        self.loss_consistency_A = self.criterion_consistency(self.feature_extractor(self.real_A), self.feature_extractor(self.rec_A))
        self.loss_consistency_B = self.criterion_consistency(self.feature_extractor(self.real_B), self.feature_extractor(self.rec_B))

        # Combine the losses
        N_A = self.real_A.numel()
        N_B = self.real_B.numel()
        R_nI = 1  # Assuming R_nI is 1 (i.e., the whole image) 
        self.loss_structure = (1 / (N_A * R_nI)) * self.loss_consistency_A + (1 / (N_B * R_nI)) * self.loss_consistency_B

        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_structure
