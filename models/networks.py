...
class UNetSegmentation(nn.Module):
    ...
    def forward(self, x):
        ...
        x_dec = self.upconv(x_enc)
        x_out = self.decoder(x_dec)
        return x_out

def compute_joint_histogram(x, y, bins=256):
    assert x.shape == y.shape
    device = x.device  # Get the device of the input tensors
    batch_size = x.size(0)
    hist_range = (0, 1)  # Assuming the input is normalized between 0 and 1
    
    joint_hist = torch.zeros((batch_size, bins, bins), device=device)
    for i in range(batch_size):
        # Flatten x and y
        x_flat = x[i].view(-1)
        y_flat = y[i].view(-1)
        
        # Compute 2D histogram using bucketize
        x_bins = torch.bucketize(x_flat, torch.linspace(hist_range[0], hist_range[1], bins + 1, device=device), right=True) - 1
        y_bins = torch.bucketize(y_flat, torch.linspace(hist_range[0], hist_range[1], bins + 1, device=device), right=True) - 1

        # Clamp the values to stay within the range [0, bins-1]
        x_bins = torch.clamp(x_bins, 0, bins-1)
        y_bins = torch.clamp(y_bins, 0, bins-1)

        for j in range(len(x_flat)):
            joint_hist[i, x_bins[j], y_bins[j]] += 1
    
    return joint_hist

def compute_probabilities(hist):
    return hist / hist.sum(dim=(1, 2), keepdim=True)

def mutual_information(x, y, bins=256):
    assert x.size() == y.size()
    
    joint_hist = compute_joint_histogram(x, y, bins)
    joint_prob = compute_probabilities(joint_hist)
    
    x_hist = torch.sum(joint_hist, dim=2)
    y_hist = torch.sum(joint_hist, dim=1)
    
    x_prob = compute_probabilities(x_hist.unsqueeze(2))
    y_prob = compute_probabilities(y_hist.unsqueeze(1))
    
    # Avoid log(0) by adding a small constant epsilon
    epsilon = 1e-10
    joint_prob = joint_prob + epsilon
    x_prob = x_prob + epsilon
    y_prob = y_prob + epsilon
    
    mi = torch.sum(joint_prob * (torch.log(joint_prob) - torch.log(x_prob) - torch.log(y_prob)), dim=(1, 2))
    return mi.mean()

def mutual_information_loss(x, y, bins=256):
    mi = mutual_information(x, y, bins)
    return -mi  # Negative because we minimize the loss
...
# MIND Descriptor for sc-CycleGAN
