...
class ShapeCycleGANModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'shape']
        ...
        self.unet_seg = networks.UNetSegmentation(in_channels=1, out_channels=1).to(self.device)
        self.unet_seg.eval() 
    ...
    def backward_G(self):
        ...
        # Compute segmented masks
        with torch.no_grad():
            segmented_fake_B = self.unet_seg(self.fake_B)
            segmented_fake_A = self.unet_seg(self.fake_A)

        # Compute Shape Loss
        self.loss_shapeB = self.criterionMask(segmented_fake_B, self.mask_A)
        self.loss_shapeA = self.criterionMask(segmented_fake_A, self.mask_B)
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_shape
