...
sct_folder = args.results_folder +'fake_B/'

# Obtain the images
sct_images = os.listdir(sct_folder)

# Remove output folder if already exists
if os.path.exists(args.final_voxels_folder + 'fake_B'):
    shutil.rmtree(args.final_voxels_folder + 'fake_B')
# Create output folder
os.makedirs(args.final_voxels_folder + 'fake_B')
...
for voxel_idx in voxels_idx:
    ...
    print('saving image ', voxel_idx)
    nib.save(nifti_img, args.final_voxels_folder + 'fake_B/' + voxel_idx +'.nii.gz')
