...
def validate(val_set, model):
    ...
    for i, data in enumerate(val_set):  # inner loop within one epoch
        ...
        if i == 4: 
            image_wandb_sct = sct.cpu().numpy()
            image_wandb_sct = np.concatenate(image_wandb_sct, axis=1)
            # Save real
            image_wandb_real = real.cpu().numpy()
            image_wandb_real = np.concatenate(image_wandb_real, axis=1)
            # Save prediction
            ...
            # Save real
            image_wandb_sct_2 = sct.cpu().numpy()
            image_wandb_sct_2 = np.concatenate(image_wandb_sct_2, axis=1)
            image_wandb_sct = np.concatenate((image_wandb_sct, image_wandb_sct_2), axis=2)
            ...
            image_wandb_sct_2 = np.concatenate(image_wandb_sct_2, axis=1)
            image_wandb_sct = np.concatenate((image_wandb_sct, image_wandb_sct_2), axis=2)
            image_wandb_sct = ((image_wandb_sct + 1) * 127.5).astype(np.uint8)
            image_wandb_sct = PIL.Image.fromarray(np.squeeze(image_wandb_sct))
            image_wandb_sct = image_wandb_sct.convert("L")
            image_wandb_real_2 = real.cpu().numpy()
            image_wandb_real_2 = np.concatenate(image_wandb_real_2, axis=1)
            image_wandb_real = np.concatenate((image_wandb_real, image_wandb_real_2), axis=2)
            image_wandb_real = ((image_wandb_real + 1) * 127.5).astype(np.uint8)
            ...
            image_wandb_pred = image_wandb_pred.convert("L")
            if wandb_run: 
                wandb.log({"val/examples": [wandb.Image(image_wandb_sct, caption="sCT"),wandb.Image(image_wandb_real, caption="CT"),wandb.Image(image_wandb_pred, caption="ssCT")]})
